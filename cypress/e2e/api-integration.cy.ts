describe('API Integration Tests', () => {
  const backendUrl = Cypress.env('BACKEND_API_URL') || 'http://localhost:3000/api'
  ...
  describe('Newsletter API', () => {
    it('should successfully subscribe to newsletter with valid email', () => {
      ...
      cy.testApiEndpoint('POST', '/newsletter', newsletterData).then((response) => {
        expect(response.status).to.eq(200)
        expect(response.body).to.have.property('success', true)
        expect(response.body).to.have.property('message')
        expect(response.body.message).to.include('subscribed')
      })
    })
  ...
  describe('Health Check', () => {
    it('should respond to health check endpoint', () => {
      cy.testApiEndpoint('GET', '/health').then((response) => {
        expect(response.status).to.eq(200)
  ...
  describe('Form Integration Tests', () => {
    ...
    it('should submit contact form through UI and verify API call', () => {
      ...
      cy.get('#contact').within(() => {
        cy.get('input[name="name"]').type('Cypress UI Test')
        cy.get('input[name="email"]').type('cypress.ui@example.com')
        cy.get('textarea[name="message"]').type('This is a test message submitted through the UI.')
